...
def plot_curve_boxplot(x_grid: np.ndarray, stats: dict, out_png: str, title: str = "", figsize=(10,6), xlim: Tuple[float,float] | None = None, ylim: Tuple[float,float] | None = None):
...
def process_folder(clip_dir: str, out_prefix: str, x_grid: np.ndarray, ylim: Tuple[float,float] | None, coverage: float, n_grid: int):
    curves = load_curves(clip_dir, label_name="shoreline")
    if not curves:
        print(f"Skipping (no curves): {clip_dir}")
        return False
    # Interpolate to the shared global grid
    ...
    plot_curve_boxplot(x_grid, stats, f"{out_prefix}_curve_boxplot.png", title=title, xlim=(x_grid.min(), x_grid.max()), ylim=ylim)
    save_band_csv(x_grid, stats, f"{out_prefix}_band_stats.csv")
...
def main():
    ...
    ap.add_argument("--out_root", default="./vis_new/", help="Output root directory for batch mode. Defaults to --root_dir if omitted.")
    ap.add_argument("--coverage", type=float, default=0.8, help="(Single-folder mode only) Fraction of curves that must overlap the x-range to define the grid (0.5–0.95 typical)")
    ...
    if args.root_dir:
        ...
        # Compute unified axes across all folders
        gxmin, gxmax, gymin, gymax = global_axes_from_folders(folders, label_name="shoreline")
        x_grid = make_grid_from_range(gxmin, gxmax, args.n_grid)
        ylim = (gymin, gymax)
        ...
        for d in folders:
            ...
            ok = process_folder(d, out_prefix, x_grid, ylim, coverage=args.coverage, n_grid=args.n_grid)
            if ok:
                processed += 1
